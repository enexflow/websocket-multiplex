...
// Configuration
const MULTIPLEXER_URL = 'ws://localhost:8080';
...
function createOcppMessage(messageType, action, payload) {
  switch (messageType) {
    ...
    case 'CALLERROR':
      return [4, payload.messageId, payload.errorCode, payload.errorDescription, payload.errorDetails];
    default:
...
console.log(`Connecting to multiplexer at ${MULTIPLEXER_URL}/${CHARGE_POINT_ID}`);
const ws = new WebSocket(`${MULTIPLEXER_URL}/${CHARGE_POINT_ID}`, [`ocpp${OCPP_VERSION}`]);
...
ws.on('open', () => {
  ...
  setTimeout(() => {
    ...
    console.log('Sending StatusNotification:', JSON.stringify(statusNotification));
  ...
  try {
    ...
    if (messageTypeId === 2) {
      ...
      if (action === 'Reset') {
        ...
        console.log('Sending RemoteStartTransaction response:', JSON.stringify(response));
        ws.send(JSON.stringify(response));
        
        // Simulate starting a transaction
        setTimeout(() => {
          const startTransaction = createOcppMessage('CALL', 'StartTransaction', {
            connectorId: 1,
          ...
          console.log('Sending StartTransaction:', JSON.stringify(startTransaction));
      ...
      console.log(`Received CALLERROR for messageId: ${messageId}, error: ${errorCode} - ${errorDescription}`);
    }
  } catch (error) {
